...
param_info_sobol = {param_name: "log" for param_name in sobol_labels}
problem_sobol = Problem(model, initial_conditions_nd, t_end, times_rec, short_labels, scales, experimental_data_df, param_info_sobol, fixed_params_sobol)
log_var_params = {param: np.log10(params[param]) for param in sobol_labels}
...
def init_worker(cf):
    ...
    PROBLEM = Problem(MODEL, cf["initial_conditions_nd"], cf["t_end"], cf["times_rec"], cf["short_labels"], cf["scales"], 
                      cf["data_df"], cf["param_info_sobol"], cf["fixed_params_sobol"])
...
sobol_best_df = pd.DataFrame(columns=sobol_labels + ["SSE"])
num_best_its = 5
for k in range(num_best_its):
    fig, axs = plot_all_profiles(times_rec, experimental_data_df, colours, short_labels, labels, scales, experimental_data_df["T_ext"],
                                 _style="scatter")
    j = np.argmin(Y)
    print(problem_sobol.sse(param_values[j]))
    sobol_best_df.loc[len(sobol_best_df)] = np.array(param_values[j].tolist() + [Y[j]])
    output_df = extract_data_from_model(times, problem_sobol._model_out, short_labels, scales)
    fig, axs = plot_all_profiles(times, output_df, colours, short_labels, labels, scales, T_e, _fig=fig, _axs=axs)
    fig.savefig(f"{res_dir}/sobol/sobol_traces_{k}.png")
    Y[j] = 1e6

sobol_best_df.to_csv(f"{res_dir}/sobol/sobol_best.csv")
...
S1_conf_sorted_by_ST = {label: S1_conf_dict[label] for label in sorted_sobol_labels}
ST_arr, S1_arr, ST_conf_arr, S1_conf_arr = np.array(list(ST_sorted.values())), np.array(list(S1_sorted_by_ST.values())), np.array(list(ST_conf_sorted_by_ST.values())), np.array(list(S1_conf_sorted_by_ST.values()))
sobol_df = pd.DataFrame({"name": sorted_sobol_labels, "S1": str_x(S1_arr), "ST": str_x(ST_arr), "S1_conf": str_x(S1_conf_arr), "ST_conf": str_x(ST_conf_arr),
